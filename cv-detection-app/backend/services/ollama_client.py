        ...
        for attempt in range(1, self.retries + 1):
            try:
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    logger.debug("POST %s attempt=%d payload_keys=%s", url, attempt, list(payload.keys()))
                    resp = await client.post(url, json=payload)
                    resp.raise_for_status()
                    text = resp.text
                    parsed = self._safe_parse_json(text)
                    return parsed
            except Exception as exc:  # noqa: BLE001 - we want to handle any transport/parsing error
                last_exc = exc
